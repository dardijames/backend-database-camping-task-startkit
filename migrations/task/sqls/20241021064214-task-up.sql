...
--     6. 用戶名稱為 透明人，Email 為 opacity0@hexschooltest.io，Role 為 USER
    ...
    ANS: INSERT INTO "USER" (name, email, role) VALUES
...
-- 1-3 刪除：刪除USER 資料表中，用 Email 找到透明人，並刪除該筆資料
    ANS: DELETE FROM "USER"
WHERE "email" = ' opcatiy0@hexschooltest.io';
-- 1-4 查詢：取得USER 資料表目前所有用戶數量（提示：使用count函式）
    ANS: SELECT COUNT(*) FROM "USER"; 
...
-- 2-1. 新增：在`CREDIT_PACKAGE` 資料表新增三筆資料，資料需求如下：
    ...
    ANS: INSERT INTO "CREDIT_PACKAGE" (name, credit_amount, price) VALUES
	('7 堂組合包方案', 1400, 7),
  ('14 堂組合包方案', 2520 , 14),
  ('21 堂組合包方案', 4800 ,21);
...
-- 2-2. 新增：在 `CREDIT_PURCHASE` 資料表，新增三筆資料：（請使用 name 欄位做子查詢）
    -- 1. `王小明` 購買 `14 堂組合包方案`
    ANS:INSERT INTO "CREDIT_PURCHASE" (user_id, credit_package_id, purchased_credits, price_paid) VALUES
	(
...
-- 3-1 新增：在`COACH`資料表新增三筆教練資料，資料需求如下：
    ...
    ANS:INSERT INTO "COACH" (user_id, experience_years) VALUES
...
-- 3-2. 新增：承1，為三名教練新增專長資料至 `COACH_LINK_SKILL` ，資料需求如下：
    -- 1. 所有教練都有 `重訓` 專長
    ANS: INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id)
VALUES
  ...
  ((SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'starplatinum@hexschooltest.io')),
   (SELECT id FROM "SKILL" WHERE name = '重訓'));
    ...
    -- 2. 教練`肌肉棒子` 需要有 `瑜伽` 專長
    ANS: INSERT INTO "COACH_LINK_SKILL" (coach_id, skill_id) VALUES
  ((SELECT id FROM "COACH" WHERE user_id = (SELECT id FROM "USER" WHERE email = 'muscle@hexschooltest.io')),
...
FROM 
  "CREDIT_PURCHASE"
INNER JOIN 
  "CREDIT_PACKAGE" ON "CREDIT_PACKAGE".id = "CREDIT_PURCHASE".credit_package_id
WHERE 
...
FROM 
  "CREDIT_PURCHASE"
WHERE 
  "CREDIT_PURCHASE".purchase_at BETWEEN '2024-11-01 00:00:00' AND '2024-11-30 23:59:59';
